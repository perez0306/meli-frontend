    ...
    return (
        <section className={styles.specsBox}>
            ...
            <div className={styles.grid}>
                {specs.map((spec: SpecsI) => spec?.value ? (
                    <div key={spec.label} className={styles.row}>
                        <span className={styles.iconCircle}>
                            <Image src={spec.icon} alt={spec.label} width={32} height={32} />
                        </span>
                        <div className={styles.label}>{spec.label}: <b>{spec.value}</b></div>
                    </div>) : <div key={spec.label} className={styles.rowPrincipal}>
                    <span className={styles.iconCircle}>
                        <Image src={spec.icon} alt={spec.label} width={32} height={32} />
                    </span>
                    <div>
                        <div className={styles.label}>{spec.label}: <b>{spec.size}</b></div>
